...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create modbot-171a
  fly apps create modbot-171a-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app modbot-171a
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app modbot-171a-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'modbot-171a-staging' does not match app name in config file 'modbot-171a'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name modbot-171a-db
  fly postgres attach --app modbot-171a modbot-171a-db

  fly postgres create --name modbot-171a-staging-db
  fly postgres attach --app modbot-171a-staging modbot-171a-staging-db
  ```
